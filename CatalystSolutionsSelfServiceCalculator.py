...
def fetch_admin_values():
    cursor.execute("SELECT key, value FROM admin_values")
    return dict(cursor.fetchall())

def update_admin_value(value, key):
    cursor.execute(
        sql.SQL("UPDATE admin_values SET value = %s WHERE key = %s"),[value, key]
    )
    conn.commit()
    
# Fetch initial values from database
admin_values = fetch_admin_values()



# Consultant Type Global Variables
W2_VALUE = admin_values['W2_VALUE']
C2C_VALUE = admin_values['C2C_VALUE']

# VMS Fee Global Variables
NO_VMS_VALUE = 0
ARIBA_VALUE = admin_values['ARIBA_VALUE']
VENDORPASS_VALUE = admin_values['VENDORPASS_VALUE']
FIELDGLASS_VALUE = admin_values['FIELDGLASS_VALUE']
ORACLE_VALUE = admin_values['ORACLE_VALUE']
VMS_VALUE = admin_values['VMS_VALUE']
BEELINE_VALUE = admin_values['BEELINE_VALUE']
HAYS_VALUE = admin_values['HAYS_VALUE']
PRIORITY_VALUE = admin_values['PRIORITY_VALUE']
...
def show_admin_panel():
    ...
    def check_password():
        if password_entry.get() == "CATALYST@IT_TECHTEAM":
...
def open_admin_panel():
    ...
    def update_values():
        ...
        try:
            ...
            update_admin_value(w2_entry.get(),'W2_VALUE')
            C2C_VALUE = float(c2c_entry.get())
            update_admin_value(c2c_entry.get(),'C2C_VALUE')
            ARIBA_VALUE = float(ariba_entry.get())
            update_admin_value(ariba_entry.get(),'ARIBA_VALUE')
            VENDORPASS_VALUE = float(vendorpass_entry.get())
            update_admin_value(vendorpass_entry.get(),'VENDORPASS_VALUE')
            FIELDGLASS_VALUE = float(fieldglass_entry.get())
            update_admin_value(fieldglass_entry.get(),'FIELDGLASS_VALUE')
            ORACLE_VALUE = float(oracle_entry.get())
            update_admin_value(oracle_entry.get(),'ORACLE_VALUE')
            VMS_VALUE = float(vms_entry.get())
            update_admin_value(vms_entry.get(),'VMS_VALUE')
            BEELINE_VALUE = float(beeline_entry.get())
            update_admin_value(beeline_entry.get(),'BEELINE_VALUE')
            HAYS_VALUE = float(hays_entry.get())
            update_admin_value(hays_entry.get(),'HAYS_VALUE')
            PRIORITY_VALUE = float(priority_entry.get())
            update_admin_value(priority_entry.get(),'PRIORITY_VALUE')
...
def check_password():
    ...
    if password == "CATALYST@IT_TECHTEAM":  # Change this to your desired admin password
